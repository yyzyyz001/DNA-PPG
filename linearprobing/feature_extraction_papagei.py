...
def save_embeddings_df(path, df, case_name, child_dirs, save_dir, model, batch_size, device, output_idx, resample=False, normalize=True, fs=None, fs_target=None, is_mt_regress=False):
    ...
    for i in tqdm(range(len(child_dirs))):
        ...
        print(f"[INFO] Saving file {case} to {save_dir}")
        joblib.dump(embeddings, os.path.join(save_dir, case + ".p"))  ## 这里是将每个被试分别存放，而不是按照子任务存放
...
if __name__ == "__main__":
    ...
    ## 这里是只对一个split进行操作
    if args.dataset in ["vital", "mimic", "mesa", "wesad", "dalia", "marsh", "ppg-bp"]:
        ...
        joblib.dump(
            dict_feat, os.path.join(os.path.join(args.save_dir, model_name), f"dict_{args.split}_{content}.p")
        ) 
    else:
