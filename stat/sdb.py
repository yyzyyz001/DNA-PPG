...
def fit(division='42'):
    ...
    y_AHI_train = data['y_AHI_train']

    y_AHI_val = data['y_AHI_val']

    y_AHI_test = data['y_AHI_test']
    ...
    y_AHI_train_val = np.concatenate([y_AHI_train, y_AHI_val])
    ...
    print("Optimizing AHI model...")
    # Grid search for AHI model
    logistic_AHI = LogisticRegression(random_state=42)
    grid_search_AHI = GridSearchCV(
        logistic_AHI,
        param_grid,
        cv=custom_cv,
        scoring='roc_auc',
    ...
    grid_search_AHI.fit(X_train_val, y_AHI_train_val)

    print(f"\nBest AHI parameters: {grid_search_AHI.best_params_}")
    print(f"Best AHI validation AUROC: {grid_search_AHI.best_score_:.3f}")

    # Train final models on training set only with best parameters
    final_logistic_AHI = LogisticRegression(**grid_search_AHI.best_params_, random_state=42)

    final_logistic_AHI.fit(X_train_scaled, y_AHI_train)

    # Make predictions on validation set
    AHI_val_pred = final_logistic_AHI.predict_proba(X_val_scaled)[:, 1]

    # Make predictions on test set
    AHI_test_pred = final_logistic_AHI.predict_proba(X_test_scaled)[:, 1]

    # Calculate validation metrics
    AHI_val_auroc = roc_auc_score(y_AHI_val, AHI_val_pred)

    # Calculate test metrics
    AHI_test_auroc = roc_auc_score(y_AHI_test, AHI_test_pred)

    print("\n=== Validation Results (with optimized hyperparameters) ===")
    print(f"AHI - AUROC: {AHI_val_auroc:.3f}")

    print("\n=== Test Results ===")
    print(f"AHI - AUROC: {AHI_test_auroc:.3f}")
    # %%
    ...
    # Bootstrap for AHI test results
    ahi_bootstrap = bootstrap_auroc(y_AHI_test, AHI_test_pred, n_bootstrap=500)

    if ahi_bootstrap is not None:
        print("AHI Test Results:")
        print(f"  Original AUROC: {AHI_test_auroc:.3f}")
        print(f"  Bootstrap Mean AUROC: {ahi_bootstrap['mean_auroc']:.3f} ± {ahi_bootstrap['std_auroc']:.3f}")
        print(f"  95% CI: [{ahi_bootstrap['ci_lower']:.3f}, {ahi_bootstrap['ci_upper']:.3f}]")
        print(f"  Valid bootstrap samples: {ahi_bootstrap['n_valid_samples']}/500")
    else:
