...
def fit(division='42'):
    ...
    for y in ('train', 'val', 'test'):
        ...
        data['y_valence_' + y] = df['valence_binary'].values
        ...
        for subject in unique_subjects:
            files = []
            for status in ('base', 'fun', 'medi1', 'medi2', 'tsst'):
                datafile_path = datafiles / (subject + '_' + status)
                files.extend([f for f in datafile_path.iterdir() if f.is_file() and f.suffix == ".p"])
    ...
    y_valence_train = data['y_valence_train']

    y_valence_val = data['y_valence_val']

    y_valence_test = data['y_valence_test']
    ...
    y_valence_train_val = np.concatenate([y_valence_train, y_valence_val])
    ...
    print("Optimizing valence_binary model...")
    # Grid search for valence_binary model
    logistic_valence = LogisticRegression(random_state=42)
    ...
    grid_search_valence.fit(X_train_val, y_valence_train_val)

    print(f"\nBest valence_binary parameters: {grid_search_valence.best_params_}")
    print(f"Best valence_binary validation AUROC: {grid_search_valence.best_score_:.3f}")

    # Train final models on training set only with best parameters
    final_logistic_valence = LogisticRegression(**grid_search_valence.best_params_, random_state=42)

    final_logistic_valence.fit(X_train_scaled, y_valence_train)

    # Make predictions on validation set
    valence_val_pred = final_logistic_valence.predict_proba(X_val_scaled)[:, 1]

    # Make predictions on test set
    valence_test_pred = final_logistic_valence.predict_proba(X_test_scaled)[:, 1]

    # Calculate validation metrics
    valence_val_auroc = roc_auc_score(y_valence_val, valence_val_pred)

    # Calculate test metrics
    valence_test_auroc = roc_auc_score(y_valence_test, valence_test_pred)

    print("\n=== Validation Results (with optimized hyperparameters) ===")
    print(f"valence_binary - AUROC: {valence_val_auroc:.3f}")

    print("\n=== Test Results ===")
    print(f"valence_binary - AUROC: {valence_test_auroc:.3f}")
    # %%
    ...
    # Bootstrap for valence test results
    valence_bootstrap = bootstrap_auroc(y_valence_test, valence_test_pred, n_bootstrap=500)

    if valence_bootstrap is not None:
        print("Valence Test Results:")
        print(f"  Original AUROC: {valence_test_auroc:.3f}")
        print(f"  Bootstrap Mean AUROC: {valence_bootstrap['mean_auroc']:.3f} ± {valence_bootstrap['std_auroc']:.3f}")
        print(f"  95% CI: [{valence_bootstrap['ci_lower']:.3f}, {valence_bootstrap['ci_upper']:.3f}]")
        print(f"  Valid bootstrap samples: {valence_bootstrap['n_valid_samples']}/500")
    else:
